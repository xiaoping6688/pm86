...
InteractorDaemonizer.daemonize = function(infos, cb) {
  ...
  child.once('message', function(msg) {
     ...
     */
    if (msg.error == true) {
      console.log(chalk.red('[PM86.io][ERROR]'), msg.msg);
      console.log(chalk.cyan('[PM86.io]') + ' Contact support contact@keymetrics.io and send us the error message');
      return cb(msg);
    }

    if (msg.km_data.disabled == true) {
      console.log(chalk.cyan('[PM86.io]') + ' Server DISABLED BY ADMINISTRATION contact support contact@keymetrics.io with reference to your public and secret keys)');
      return cb(msg);
    }

    if (msg.km_data.error == true) {
      console.log(chalk.red('[PM86.io][ERROR]') + ' ' + msg.km_data.msg + ' (Public: %s) (Secret: %s) (Machine name: %s)', msg.public_key, msg.secret_key, msg.machine_name);
      return cb(msg);
    }

    else if (msg.km_data.active == false && msg.km_data.pending == true) {
      console.log(chalk.red('[PM86.io]') + ' ' + chalk.bold.red('Agent PENDING') + ' - Web Access: https://app.keymetrics.io/');
      console.log(chalk.red('[PM86.io]') + ' You must upgrade your bucket in order to monitor more servers.');

      return cb(msg);
    }

    if (msg.km_data.active == true) {
      console.log(chalk.cyan('[PM86.io]') + ' [%s] Agent ACTIVE - Web Access: https://app.keymetrics.io/',
...
InteractorDaemonizer.launchAndInteract = function(opts, cb) {
  ...
  InteractorDaemonizer.getSetKeys({
    ...
    console.log(chalk.cyan('[PM86.io]') + ' Using (Public key: %s) (Private key: %s)', data.public_key, data.secret_key);
